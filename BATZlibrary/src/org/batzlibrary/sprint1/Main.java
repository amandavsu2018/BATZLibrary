...
public class Main {
	...
	public static void main(String[] args) {
		...
		// use while loop with sessionOpen and timestamp to determine if Login
		// hasn't timed-out
		while (sessionOpen == true) {
			...
			if (status.equals("member")) {
				...
				System.out.println("Your status is an associate. Nothing to do yet so logging out.");
		...
		if (createuser.equals("")) {
			...
			try {
				...
				String query = "INSERT INTO users (user_username, user_password, user_pin, user_locked, user_status, user_firstname, user_lastname, user_street, user_city, user_state, user_zip, user_phone, user_email)"
						+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
